...
class GenerateDevelopmentCodeTemplateCommand extends Command {
    protected $table, $namespace, $name, $read;
    ...
    public function generateMigration () {
        ...
        $template = file_get_contents(app_path() . '/Templates/MigrationTemplate.stub');
    ...
    public function generateResourceFiles () {
        $content = $this->replaceContents(file_get_contents(app_path() . '/Templates/ObserverTemplate.stub'));
        ...
        $content = $this->replaceContents(file_get_contents(app_path() . '/Templates/ModelTemplate.stub'));
        $file = app_path() . '/Models/' . $this->namespace . '/' . $this->name . '.php';
        file_put_contents($file, $content);
        $this->info('Created Model File : ' . str_replace(app_path(), '', $file));

        $content = $this->replaceContents(file_get_contents(app_path() . '/Templates/ControllerTemplate.stub'));
